...
export async function prefetchCollection<T>(
  ...
  const constraints =
    typeof queryParams === 'object' && queryParams && 'constraints' in queryParams
      ? (queryParams as { constraints?: unknown[] }).constraints
      : [];
  await queryClient.prefetchQuery({
    queryKey,
    queryFn: () => getDocuments<T>(collectionName, constraints as import('firebase/firestore').QueryConstraint[]),
    staleTime: 5 * 60 * 1000, // 5 minutes
  });
}
...
export function updateCachedDocument<T>(
  collectionName: string,
  docId: string,
  updateFn: (_oldData: T | undefined) => T
): void {
