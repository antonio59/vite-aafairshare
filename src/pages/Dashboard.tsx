...
import { convertToSchemaUser } from "@shared/types";
...
export default function Dashboard() {
  ...
  const safeUsers = useMemo(() => {
    ...
    if (currentUser && !users.some(u => u.id === currentUser.uid)) {
      ...
      users.push({
        ...
        displayName: currentUser.displayName?.split(' ')[0] || currentUser.email?.split('@')[0] || 'Current User',
    ...
    // Convert all users to SchemaUser type
    return users.map(user => convertToSchemaUser(user));
  ...
  const fetchExpenses = useCallback(async (month: string): Promise<ExpenseWithDetails[]> => {
    ...
    try {
      ...
      const expenseUserIds = new Set(snapshot.docs.map(doc => doc.data().paidByUserId));
      ...
      const resolvedExpenses = snapshot.docs.map((expenseDoc): ExpenseWithDetails => {
        ...
        let user = userMap.get(expenseData.paidByUserId);
        
        // Handle missing user more robustly
        if (!user) {
          console.warn(`Dashboard: User not found for expense ${expenseDoc.id}, paidByUserId:`, expenseData.paidByUserId);
        ...
        // Ensure we have a valid paidByUser with username field
        const paidByUser = {
          id: user.id,
        ...
        // Debug the actual date value from Firestore
        console.log(`Processing date for expense ${expenseDoc.id}:`, expenseData.date);
        
        // Handle different date formats from Firestore
        if (expenseData.date instanceof Timestamp) {
          // Direct Timestamp instance
          date = expenseData.date.toDate();
        } else if (expenseData.date && typeof expenseData.date === 'object') {
          if (typeof expenseData.date.toDate === 'function') {
            // Object with toDate method (Firestore Timestamp)
            date = expenseData.date.toDate();
          } else if (expenseData.date.seconds !== undefined) {
            // Firestore Timestamp-like object with seconds
            date = new Date(expenseData.date.seconds * 1000);
          } else {
            // Log the structure of the date object for debugging
            console.error(`Unexpected date object structure for expense ${expenseDoc.id}:`, JSON.stringify(expenseData.date));
          ...
          // Log error but don't fallback - this should never happen with proper validation
          console.error(`Missing or invalid date for expense ${expenseDoc.id}`, expenseData.date);
          // Use the month to create a date in that month rather than today's date
          const [year, month] = expenseData.month.split('-').map(Number);
          date = new Date(year, month - 1, 15); // Use middle of the month as fallback
        }
        
        // Just validate and log errors, but don't replace with current date
        if (!(date instanceof Date) || isNaN(date.getTime())) {
          console.error(`Invalid date conversion for expense ${expenseDoc.id}. Original:`, expenseData.date);
          // Keep the invalid date to surface the issue rather than hiding it
        }
        ...
        return { ...expenseBase, category, location, paidByUser: paidByUser };
      });

      resolvedExpenses.sort((a, b) => {
        // Safely handle sorting - make sure both dates are valid dates
        ...
        return bTime - aTime; // Sort by date descending, handling invalid dates
      });
  ...
  const calculatedSummary = useMemo(() => {
    ...
    // Use explicit expenses data rather than hook variable
    const expenses = expensesData ?? [];
    ...
    expenses.forEach(exp => {
      ...
      if (exp.paidByUserId === user1Id) userExpenses[user1Id] += amount;
      else if (user2Id && exp.paidByUserId === user2Id) userExpenses[user2Id] += amount;
      
      // Handle split types
      if (exp.splitType === "50/50") { 
        totalSplitExpenses += amount; 
        if (exp.paidByUserId === user1Id) user1_paid_50_50 += amount; 
      }
      else if (exp.splitType === "100%") { 
        if (exp.paidByUserId === user1Id) user1_paid_100_owed_by_other += amount; 
        else if (user2Id && exp.paidByUserId === user2Id) user2_paid_100_owed_by_other += amount; 
      }
    });
  ...
  }, [expensesData, user1Id, user2Id, currentMonth]);
  ...
  useEffect(() => {
    ...
    const fetchSettlementStatus = async () => {
      try {
        // Fix collection name - should be settlements, not expenses
        ...
        // Only update state if the component is still mounted
        if (isMounted) {
          // Be more specific about permission issues for debugging
          ...
          // Don't show error toast for permission issues - they're expected during development
  ...
  const handleExport = useCallback(async (format: ExportFormat) => {
    ...
    try { 
      ...
      // Create an array of objects that conform to the schema User type
      const convertedUsers = (allUsers || []).map(u => ({
        id: u.id,
      ...
      // @ts-ignore - Type mismatch between the two different User interfaces
      exportExpenses({ 
        format, 
        month: currentMonth, 
        expenses, 
  ...
  useEffect(() => {
    if (expenses.length > 0) {
      const uniqueUserIds = new Set(expenses.map(exp => exp.paidByUserId));
      ...
      const missingUserIds = Array.from(uniqueUserIds).filter(id => !safeUserIds.includes(id as string));
