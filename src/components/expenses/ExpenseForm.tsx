...
import { ExpenseWithDetails, Category, Location, User, Expense } from '../../types/expense';
...
export default function ExpenseForm({ expense = null, onClose }: ExpenseFormProps) {
  ...
  const [date, setDate] = useState<Date>(expense?.date ? new Date(expense.date instanceof Timestamp ? expense.date.toDate() : expense.date) : new Date());
